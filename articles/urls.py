...
urlpatterns = [
    ...
    path("<int:article_id>/", views.ArticleDetailView.as_view()),  # 게시글 R -
    path("categoryplus/", views.CategoryCreateView.as_view()),  # 카테고리 생성 - 완료
    ...
    path("ingredient/", views.IngredientView.as_view()),  # 레시피 재료 C - 완료
    path(
        "<int:article_id>/<int:ingredient_id>/", views.IngredientDetailView.as_view()
    ),  # 레시피 재료 RUD - 완료
